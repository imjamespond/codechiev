...
__thread Thread* this_thread__(NULL);
...
Thread::GetCurrentThreadName()
{
    if(this_thread__)
    {
        return this_thread__->getThreadName();
    }
...
int main_thread_start__()
{
    return Thread::GetCurrentThreadId();
}
int main_thread_id__ = main_thread_start__();

int
Thread::GetMainId()
{
    return main_thread_id__;
}
...
public:
  ...
  inline void set_write_action(bool set = true) { action = (2 | action) ^ (set ? 0 : 2); }
  ...
  inline int write_action() { return action & 2; }
  inline int read_disabled() { return action & 4; } 
...
private:
  const int sockfd;
  int events;
  int action; //2 for switch to edge, 4 for disable read
};
} // namespace net
} // namespace codechiev
